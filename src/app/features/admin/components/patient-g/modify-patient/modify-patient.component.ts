...
export class ModifyPatientComponent implements OnInit {
  ...
  async getPatient():Promise<any>{
    ...
    this.form_patient.patchValue({name_patient:patient.patient.name_patient});
    this.form_patient.patchValue({lastname_patient:patient.patient.lastname_patient});
    this.form_patient.patchValue({sexo_patient:patient.patient.sexo_patient});
    this.form_patient.patchValue({telefono_patient:patient.patient.telefono_patient});
    this.form_patient.patchValue({direccion_patient:patient.patient.direccion_patient});
  }
  ...
  async edit():Promise<any> {
    try{
      setTimeout(async () => {
        ...
        const editEvent = await this.patientService.editPatient(this.id_edit,this.form_patient.value);
